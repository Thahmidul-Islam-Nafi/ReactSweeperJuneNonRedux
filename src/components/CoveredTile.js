...
import {BoardContext,SetBoardContext,SetMinesContext,CountedMinesContext,FlagONContext,GameOverContext, SetGameOverContext} from './PreApp';
import { getNeighbours,mapToDoubleIndex,mapToSingleIndex } from '../Boardlogic';
...
const CoveredTile = ({index})=>{
    ...
    const style = {backgroundColor:"cyan",color:"red", height:"30px",width:"30px",textAlign:"center"}
    ...
    function setTileFlag(indTile){
        ...
        if(MineNumber>0){
            setBoard((preBoard)=>{
                ...
                const modifiednewBoardTile = {...newBoardTile,flag:!newBoardTile.flag}
    ...
    function RevealCells (CellIndex){
        ...
        if(board[CellIndex].uncovered===false&&board[CellIndex].flag===false){
        ...
        if(board[CellIndex].value==="0"){
            ...
            setTile(CellIndex)
            let neighbouringTiles = getNeighbours(mapToDoubleIndex(CellIndex),board)[1]
    ...
    return (
        <div style= {style} onClick={()=>{FlagON?setTileFlag(index):RevealCells(index)}}>{board[index].flag?"F":""}</div>
